...
public abstract class MixinLavaFluid {
                    ...
                    if (blockState.isAir()) {
                        if (this.hasFlammableNeighbours(level, blockPos)) {
                            ...
                            if (level.getBlockState(blockPos).getBlock() != Blocks.FIRE) {
                                if (org.bukkit.craftbukkit.v1_20_R4.event.CraftEventFactory.callBlockIgniteEvent(level, blockPos, pos).isCancelled()) {
                                    continue;
                                }
                            }
                            // CraftBukkit end
    ...
    private boolean banner$spreadTo(LevelAccessor instance, BlockPos pos, BlockState state, int i) {
        return org.bukkit.craftbukkit.v1_20_R4.event.CraftEventFactory.handleBlockFormEvent(instance.getMinecraftWorld(), pos, Blocks.STONE.defaultBlockState(), 3);
    }
